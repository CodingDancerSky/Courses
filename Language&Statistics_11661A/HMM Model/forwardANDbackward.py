...
def backward(data, transition_probs, sum_total):
    beta = genMatrix(2, sum_total)
    beta[1][0] = 1.0
    for t in range(1, sum_total):
        index = sum_total - 1 - t
        beta[0][t] = beta[0][t - 1] * transition_probs[0][0] * getEmission(1, data[index]) \
                     + beta[1][t - 1] * transition_probs[0][1] * getEmission(1, data[index])
        beta[1][t] = beta[1][t - 1] * transition_probs[1][1] * getEmission(1, data[index]) \
                     + beta[0][t - 1] * transition_probs[1][0] * getEmission(0, data[index])
...
def get_alphabeta():
    ...
    alpha = forward(data, transition_probs,sum_total)
    beta = backward(data, transition_probs,sum_total)
