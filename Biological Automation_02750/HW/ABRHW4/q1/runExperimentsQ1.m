...
CALGeneralizationError = sum(TRUE_LABELS)/numsamples; % compute generalization error
...
for (i=1:numsamples)
    ...
    [hPos flagPos] = learnQ1(DATA(1:x),SLabelsPos(1:x));
    [hNeg flagNeg] = learnQ1(DATA(1:x),SLabelsNeg(1:x));
    ...
    if(flagPos == 0 && flagNeg == 0)
        ...
        CALGeneralizationError = [CALGeneralizationError, newError];
        ...
        if(flagPos == 0)
            ...
            % without noise, it's garanteed to have at one 0.
            SLabels(x) = 0;
            h = hNeg;
        end
    end
